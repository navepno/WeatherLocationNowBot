...
# todo: сделать норм db | сделать функию редактирования уведомлений
...
def get_weather(chat_id, location, token=WEATHER_TOKEN):
    try:
...
def help_message(message):
    ...
    bot.send_message(chat_id, 'Чем я могу помочь?')
...
def get_time_to_send(message):
    ...
    try:
        ...
        elif len(user_time.split(':')) != 2:
            print(2)
            again = bot.send_message(chat_id, '2Неверный формат времени\nНапиши в формате: 08:21')
            bot.register_next_step_handler(again, get_time_to_send)
        elif not (0 <= int(user_time.split(':')[0]) < 24) or not (0 <= int(user_time.split(':')[1]) < 60):
            print(3)
...
def get_days_to_send(call):
    ...
    # print(call)
    answers = {0: ['Понедельникам', 'Понедельник'],
               1: ['Вторникам', 'Вторник'],
               2: ['Средам', 'Среда'],
               3: ['Четвергам', 'Четверг'],
               4: ['Пятницам', 'Пятница'],
               5: ['Субботам', 'Суббота'],
               6: ['Воскресеньям', 'Воскресенье']}
    out = []
    user = user_dict[chat_id]
    print(user.days)
    for i in call.option_ids:
        if i in answers:
            ...
            user.days[answers[i][1]] = user.time
...
def get_end(call):
    ...
    if call.data == 'yes':
        ...
        print(user.days)
        stop_markup = telebot.types.ReplyKeyboardRemove()
        text = bot.send_message(chat_id, 'Супер!\nБот запущен, хорошего пользования',
                                reply_markup=stop_markup)
        user.ready = 'YES'
    schedule.every(int(3)).seconds.do(get_weather, chat_id, user.location)
    while user.ready == 'YES':
        print('- - - - - - - - -')
...
def edit_location(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, 'пока не умею')
...
def get_my_notifications(message):
    ...
    for i in days:
        if days[i] != '':
            text_message += f'{i} - {days[i]}\n'
    bot.send_message(chat_id, f'Твои уведомления:\n\n{text_message}')
...
def get_weather_now(message):
    chat_id = message.chat.id
    user = user_dict[chat_id]
    bot.send_message(chat_id, 'weather now')
    get_weather(chat_id, user.location)
    # user.test = 60
    # text_message = get_weather(user.location)
...
def stop_notification(message):
    ...
    user = user_dict[chat_id]
    user.ready = 'YES'
    bot.send_message(chat_id, 'Уведомления восстановлены')
    schedule.every(int(3)).seconds.do(get_weather, chat_id, user.location)
    while user.ready == 'YES':
        print('+ - - - - - - - -')
        schedule.run_pending()
        sleep(1)
    # bot.register_next_step_handler(restart, get_end)
